    ...
    <main className="min-h-[100dvh] overflow-x-hidden bg-zinc-950 text-zinc-100">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 border-b border-white/10 bg-zinc-950/80 backdrop-blur-md">
        <div className="mx-auto flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          {/* Right: empty spacer on mobile, logo-only */}
          <div />
        </div>
      </header>
      ...
      <div className="mx-auto flex max-w-7xl gap-6 px-4 pt-20 sm:px-6 lg:px-8 lg:py-6 lg:pt-24">
        <aside
          className={`fixed inset-y-0 left-0 z-40 w-[86vw] max-w-72 rounded-r-[2rem] border-r border-white/10 bg-zinc-900/95 p-4 pt-24 shadow-[0_20px_60px_-20px_rgba(0,0,0,0.8)] backdrop-blur-xl transition-transform duration-300 lg:sticky lg:top-24 lg:w-72 lg:self-start lg:max-h-[calc(100dvh-8rem)] lg:translate-x-0 lg:rounded-[2rem] lg:border lg:bg-zinc-900/80 lg:pt-6 overflow-visible h-full lg:h-auto ${
        ...
        >
          <div className="flex h-full flex-col">
            ...
            {/* ── User dropdown (pushed to bottom) ────────────────────────── */}
            <div className="relative mt-auto pt-4" ref={userMenuRef}>
              <button
                type="button"
                onClick={() => setUserMenuOpen((open) => !open)}
                className="flex w-full items-center gap-3 rounded-[1.5rem] border border-white/10 bg-zinc-950/70 p-3 text-left transition hover:border-gold-400/20 hover:bg-gold-400/10"
              >
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gold-400/20 text-xs font-bold uppercase text-gold-400">
              ...
              {userMenuOpen && (
                <div className="absolute bottom-full left-0 right-0 mb-2 rounded-[1.5rem] border border-white/10 bg-zinc-900 p-2 shadow-xl backdrop-blur-xl">
